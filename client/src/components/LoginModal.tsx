...
import { Phone, KeyRound, Loader2, CheckCircle2, RefreshCw, ArrowRight } from "lucide-react";
...
export default function LoginModal({ open, onClose }: LoginModalProps) {
  ...
  const [normalizedPhone, setNormalizedPhone] = useState(""); // E.164 returned by server
  ...
  const sendOtp = trpc.auth.sendOtp.useMutation({
    onSuccess: (data) => {
      setNormalizedPhone(data.phone); // E.164 from server
      setStep("otp");
      startResendTimer();
  ...
  const verifyOtp = trpc.auth.verifyOtp.useMutation({
    ...
    onError: (e) => {
      ...
      // Clear code on wrong attempt so user can re-enter
      setCode("");
      ...
      <DialogContent className="max-w-sm mx-auto" dir="rtl">
        ...
        {step === "phone" ? (
          <div className="space-y-4 pt-1">
            {/* Phone input */}
            <div>
              <label className="text-sm font-medium text-foreground block mb-1.5">מספר טלפון</label>
              ...
              <p className="text-xs text-muted-foreground mt-1">לדוגמה: 050-1234567 או +972501234567</p>
            </div>

            {/* Optional name */}
            <div>
              <label className="text-sm font-medium text-foreground block mb-1.5">שם מלא (אופציונלי)</label>
              <Input
            ...
            >
              {sendOtp.isPending ? (
                ...
                <>שלח קוד<ArrowRight className="h-4 w-4 mr-2" /></>
              )}
            </Button>
          ...
          <div className="space-y-4 pt-1">
            {/* OTP code input */}
            <div>
              <label className="text-sm font-medium text-foreground block mb-1.5">קוד אימות</label>
              <Input
                ref={codeInputRef}
                type="text"
                inputMode="numeric"
